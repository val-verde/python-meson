...
class DependencyScanner:
    def __init__(self, pickle_file: str, outfile: str, sources: T.List[str]):
        ...
        self.submodules = {} # type: T.Dict[str, T.List[str]]
    ...
    def scan_fortran_file(self, fname: str) -> None:
        ...
        for line in fpath.read_text().split('\n'):
            ...
            if submodule_export_match:
                ...
                try:
                    self.submodules[parent_module_name].append(submodule_name)
                except KeyError:
                    self.submodules[parent_module_name] = [submodule_name]
                self.sources_with_exports.append(fname)
                self.provided_by[concat_name] = fname
                self.exports[fname] = concat_name
                # submodule (parent) parent   <- does require parent.mod
                # submodule (a1:a2) a3        <- does require a1@a2.smod
                if ':' in parent_module_name_full:
                    if fname in self.needs:
        ...
        with open(self.outfile, 'w') as ofile:
            ...
            for src in self.sources:
                ...
                if src in self.needs:
                    ...
                    for submodule_name in self.submodules.get(modname, []):
                        concat_name = '{}:{}'.format(modname, submodule_name)
                        mods_and_submods_needed.append(concat_name)
                ...
                if module_files_generated:
                    mod_gen = '| ' + ' '.join(module_files_generated)
                else:
                    mod_gen = ''
                if module_files_needed:
                    mod_dep = '| '  + ' '.join(module_files_needed)
                else:
                    mod_dep = ''
